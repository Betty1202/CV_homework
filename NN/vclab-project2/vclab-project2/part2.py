...
def three_layer_convnet(x, params):
    ...
    def conv2d(x, weight, bias, stride, pad):
        ...
        x_pad = torch.zeros(n, c, h_in + 2 * pad, w_in + 2 * pad).to(x.device)  # 对输入进行补零操作
        if pad > 0:
    ...
    def pooling(x, w_height, w_width, stride):
        n, c, in_height, in_width = x.shape
        ...
        for i in range(out_height):
            for j in range(out_width):
                ...
                out[:, :, i, j] = torch.max(torch.max(x[:, :, start_i: end_i, start_j: end_j], dim=-1)[0], dim=-1)[0]
        return out

    conv1_hidden = conv2d(x, conv_w1, conv_b1, 1, int((conv_w1.shape[-1] - 1) / 2))
    conv1_relu = relu_forward(conv1_hidden)
    # conv1_pool = pooling(conv1_relu, 2, 2, 2)

    conv2_hidden = conv2d(conv1_relu, conv_w2, conv_b2, 1, int((conv_w2.shape[-1] - 1) / 2))
    conv2_relu = relu_forward(conv2_hidden)
    # conv2_pool = pooling(conv2_relu, 2, 2, 2)

    N = conv2_relu.shape[0]
    hidden = conv2_relu.reshape(N, -1)
    scores = hidden @ fc_w + fc_b
...
conv_b1 = torch.zeros((channel_1,)).to(device)  # out_channel
conv_b1.requires_grad = True
conv_w2 = random_weight((channel_2, channel_1, 3, 3))  # [out_channel, in_channel, kernel_H, kernel_W]
conv_b2 = torch.zeros((channel_2,)).to(device)  # out_channel
